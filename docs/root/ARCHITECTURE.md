...
- Config: environment and runtime configuration modules (for example CORS and app constants)
- Models: schema definition, indexes, persistence logic
- Services: domain/business operations with side effects (for example cache orchestration, notification emission)
- Utilities: stateless shared helpers (pagination, validation, formatting, media helper wrappers, logging helpers)
...
- Aggregation construction is now extracted into dedicated query services under `src/services/queries` (`videoQuery`, `userQuery`, `commentQuery`, `likeQuery`, `playlistQuery`, `reportQuery`, `notificationQuery`) to keep controllers thin and composable
